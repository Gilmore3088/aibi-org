...
- ✓ Conditional UI on `/education`: when the user is enrolled in AiBI-P, the hero renders a Marginalia "Your progress" card with the live `completed_modules` count and a "Resume the program" CTA. Real personalization driven from Supabase data, gracefully omitted when unenrolled.
