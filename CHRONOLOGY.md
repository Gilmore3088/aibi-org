...
| 2026-05-20 | [`docs/reviews/security-audit-2026-05-20.md`](./docs/reviews/security-audit-2026-05-20.md) | REVIEW | ACTIVE | Pre-launch "ship it" security re-audit (API auth, secrets, rate limiting, validation, orphaned files + RLS/deps gaps). Fixed 3 quick wins: removed `/api/user-profile` unauth PII fallback (→401), reset `/api/capture-email` rate limit 50→5, patched `ws` advisory. Open blockers: Next.js 14 has 4 high `npm audit` advisories (needs breaking bump); confirm `public/artifacts/*` ungating intent. Tracked: Upstash rate-limit migration, `sandbox/chat` client-supplied systemPrompt. |
