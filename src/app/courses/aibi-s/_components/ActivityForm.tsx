...
// ActivityForm — Interactive activity form for AiBI-S enrolled learners.
// Adapted from AiBI-P ActivityForm with cobalt accent color system.
...
import type { Activity, ActivityField } from '@content/courses/aibi-s';
            ...
            {(field.options ?? []).map((opt) => (
              <label key={opt.value} className="flex items-center gap-2">
                <input
                  ...
                  readOnly
                  disabled
                  className="w-4 h-4 accent-[color:var(--color-cobalt)]"
                />
...
function InteractiveField({
  ...
  const baseInputClass =
    'w-full border rounded-sm px-3 py-2 text-sm font-sans bg-white text-[color:var(--color-ink)] placeholder:text-[color:var(--color-dust)] focus:outline-none focus:ring-2 focus:ring-[color:var(--color-cobalt)] transition-shadow';
              ...
              {(field.options ?? []).map((opt) => (
                <label key={opt.value} className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="w-4 h-4 accent-[color:var(--color-cobalt)] focus:ring-2 focus:ring-[color:var(--color-cobalt)]"
                  />
                  <span className="text-sm font-sans text-[color:var(--color-ink)]">{opt.label}</span>
                </label>
              ))}
...
export function ActivityForm({
  ...
  const handleSubmit = useCallback(
    ...
    [activity.fields, activity.id, enrollmentId, weekNumber, onSubmitSuccess, state.values],
  );
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="mt-4 pt-4 border-t border-[color:var(--color-parch-dark)]">
            <button
              ...
              className="px-6 py-2.5 bg-[color:var(--color-cobalt)] hover:opacity-90 disabled:bg-[color:var(--color-parch-dark)] disabled:text-[color:var(--color-dust)] text-[color:var(--color-linen)] text-[11px] font-mono uppercase tracking-widest rounded-sm transition-opacity disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-[color:var(--color-cobalt)] focus:ring-offset-2"
