  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-8 py-20">
      ...
      <nav aria-label="Breadcrumb" className="mb-8">
        <Link
          href="/education"
          className="font-serif-sc text-[11px] uppercase tracking-[0.18em] text-[color:var(--color-ink)]/50 hover:text-[color:var(--ink-2)] transition-colors"
        >
          Education
        </Link>
        <span className="mx-2 text-[color:var(--color-ink)]/20">/</span>
        <span className="font-serif-sc text-[11px] uppercase tracking-[0.18em] text-[color:var(--ink-2)]">
          AiBI-L
        </span>
      </nav>
      ...
      <section className="mb-28" aria-labelledby="workshop-heading">
        ...
        >
          Banking AI<br />
          <span className="text-[color:var(--ink-2)] italic">Leader</span>
        </h1>
        ...
        <div className="flex flex-wrap gap-x-10 gap-y-3 mb-12">
          {[
            ...
            <div key={label} className="flex items-center gap-2">
              ...
              <span className="font-mono text-[9px] text-[color:var(--ink-2)] tabular-nums">
                {value}
              </span>
            </div>
          ))}
        ...
        <div className="flex flex-wrap gap-4">
          <Link
            ...
            className="bg-[color:var(--ink-2)] hover:opacity-90 text-[color:var(--color-linen)] px-10 py-5 rounded-sm font-bold text-[10px] uppercase tracking-[0.15em] transition-opacity flex items-center gap-3 font-mono focus:outline-none focus:ring-2 focus:ring-[color:var(--ink-2)] focus:ring-offset-2"
          >
            Request Workshop
          ...
          <a
            ...
            className="border border-[color:var(--ink-2)]/20 text-[color:var(--color-ink)] px-10 py-5 rounded-sm font-bold text-[10px] uppercase tracking-[0.15em] hover:bg-[color:var(--color-parch)] transition-colors font-mono focus:outline-none focus:ring-2 focus:ring-[color:var(--ink-2)] focus:ring-offset-2"
          >
            View Sessions
          </a>
        </div>
      ...
      <section className="mb-28" aria-labelledby="pricing-heading">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-[color:var(--ink-2)]/10 border border-[color:var(--ink-2)]/10 rounded-sm overflow-hidden">
      ...
      <section className="mb-28" aria-labelledby="sessions-heading">
        <div className="mb-14">
          ...
          <div className="w-12 h-px bg-[color:var(--ink-2)] mb-5" aria-hidden="true" />
        ...
        <div className="space-y-4" role="list" aria-label="Workshop sessions">
          {sessions.map((session) => (
            <Link
              ...
              role="listitem"
              className="block bg-[color:var(--color-parch)] border border-[color:var(--ink-2)]/10 rounded-sm p-8 hover:border-[color:var(--ink-2)]/30 transition-colors group focus:outline-none focus:ring-2 focus:ring-[color:var(--ink-2)] focus:ring-offset-2"
            >
              <div className="flex items-start justify-between gap-6">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-3">
                    <span className="font-mono text-[11px] tabular-nums text-[color:var(--ink-2)]">
                      S{session.number}
                    </span>
                    <div className="h-px flex-1 max-w-[3rem] bg-[color:var(--ink-2)]/20" aria-hidden="true" />
                  ...
                  <p className="font-sans text-xs text-[color:var(--color-slate)]">
                    <span className="font-mono text-[9px] uppercase tracking-[0.15em] text-[color:var(--ink-2)]">
                      Deliverable:
                    </span>{' '}
                    {session.deliverable}
                  </p>
                ...
                <svg
                  className="w-4 h-4 text-[color:var(--ink-2)] opacity-0 group-hover:opacity-100 transition-opacity shrink-0 mt-2"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                  aria-hidden="true"
                >
      ...
      <section className="mb-28" aria-labelledby="schedule-heading">
        <div className="bg-[color:var(--color-parch)] p-8 sm:p-12 border border-[color:var(--ink-2)]/10 rounded-sm">
          <h2
            id="schedule-heading"
          ...
          <div className="w-12 h-px bg-[color:var(--ink-2)] mb-8" aria-hidden="true" />
          ...
          <div className="space-y-0" role="list" aria-label="Day schedule">
            {SCHEDULE.map(({ time, label, duration }) => (
              <div
                key={time}
                role="listitem"
                className="flex items-baseline gap-4 py-3 border-b border-[color:var(--ink-2)]/8 last:border-b-0"
              >
                <span className="font-mono text-xs tabular-nums text-[color:var(--ink-2)] w-20 shrink-0">
                  {time}
                </span>
        ...
        >
          {WORKSHOP_DELIVERABLES.map((deliverable) => (
            <div
              key={deliverable.id}
              role="listitem"
              className="bg-[color:var(--color-parch)] border border-[color:var(--ink-2)]/10 rounded-sm p-6"
            >
              <div className="flex items-center gap-2 mb-3">
                <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-[color:var(--ink-2)]">
      ...
      <section className="text-center py-16" aria-labelledby="cta-heading">
        ...
        <Link
          href="/courses/aibi-l/request"
          className="inline-flex items-center gap-3 bg-[color:var(--ink-2)] hover:opacity-90 text-[color:var(--color-linen)] px-10 py-5 rounded-sm font-bold text-[10px] uppercase tracking-[0.15em] transition-opacity font-mono focus:outline-none focus:ring-2 focus:ring-[color:var(--ink-2)] focus:ring-offset-2"
        >
          Request Workshop
          <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
            <path fillRule="evenodd" d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z" clipRule="evenodd" />
