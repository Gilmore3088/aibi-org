...
export function PromptWizard({
  ...
  return (
    <div style={card}>
      ...
      <p style={{ color: SLATE, fontSize: '1rem', lineHeight: 1.6, margin: '8px 0 14px' }}>
      ...
      <p style={{ color: INK, fontSize: '1rem', fontWeight: 700, lineHeight: 1.4, margin: '4px 0 16px' }}>
        {scenario.memberQuestion}
      </p>

      {/* The source */}
      <div style={eyebrow}>Your only source — tell the AI to use this, and nothing else</div>
      <p style={{ color: INK, fontSize: '1rem', lineHeight: 1.6, margin: '4px 0 0', background: CREAM, padding: 14, borderRadius: 10, border: `1px solid ${LINE}` }}>
      ...
      <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', alignItems: 'flex-start', marginTop: 14 }}>
        ...
        <div style={{ flex: '1 1 320px', minWidth: 280 }}>
          ...
          >
            ...
            <p style={{ margin: 0, color: INK, fontSize: '0.8125rem', lineHeight: 1.5 }}>
              {scenario.starterPrompt}
            </p>
          </div>
          <label htmlFor="pw-input" style={{ display: 'block', fontFamily: INTER, fontSize: '0.875rem', fontWeight: 700, color: INK, marginBottom: 6 }}>
      ...
      {done && (
        <div style={{ marginTop: 18, borderTop: `1px solid ${LINE}`, paddingTop: 14 }}>
          {solved ? (
            <p style={{ color: EMERALD, fontSize: '1rem', lineHeight: 1.6, fontWeight: 700, margin: '0 0 12px' }}>
              ✓ {scenario.winLine}
            </p>
          ) : (
            <p style={{ color: SLATE, fontSize: '1rem', lineHeight: 1.6, margin: '0 0 12px' }}>
          ...
          {!isGraded ? (
            ...
            <>
              ...
              {serverError && (
                <p style={{ color: RED, fontSize: '0.875rem', marginTop: 10 }}>{serverError}</p>
              )}
            </>
          )}
        </div>
        ...
        {scenario.elements.map((el) => {
          ...
          return (
            <li key={el.key} style={{ display: 'flex', gap: 8, alignItems: 'baseline' }}>
              <span style={{ color: ok ? EMERALD : RED, fontWeight: 800, fontSize: '1rem' }}>
                {ok ? '✓' : '✗'}
              </span>
              <span style={{ fontFamily: INTER, fontSize: '1rem', lineHeight: 1.6, color: INK }}>
                <strong>{el.label}.</strong> {el.oneLiner}
                {!ok && showHints && (
                  <span style={{ color: GOLD_DEEP, display: 'block', fontSize: '0.8125rem', marginTop: 2 }}>
                    Try: {el.missingHint}
                  </span>
                )}
              </span>
  ...
  return (
    <div style={{ marginTop: 14 }}>
      ...
      <div style={{ background: CREAM, borderRadius: 12, padding: 14, borderLeft: `3px solid ${GOLD_DEEP}` }}>
        <p style={{ margin: 0, fontFamily: INTER, fontSize: '1rem', lineHeight: 1.65 }}>
          {scenario.elements.map((el) => {
            const ok = attempt.present[el.key];
            return (
      ...
      {anyBroken && (
        <p style={{ fontSize: '0.8125rem', color: RED, margin: '6px 0 0', fontFamily: INTER }}>
