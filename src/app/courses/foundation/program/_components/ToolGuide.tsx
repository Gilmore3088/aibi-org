        ...
        <ol style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {guide.gettingStarted.steps.map((step, i) => (
            <li key={i} style={{ display: 'flex', gap: 12 }}>
              <span
                style={{ fontFamily: MONO_STACK, fontSize: 11, flexShrink: 0, marginTop: 2, width: 20, textAlign: 'right', color: guide.colorVar }}
                aria-hidden="true"
              >
                {i + 1}.
              </span>
              <span style={{ fontSize: 16, color: 'var(--ink)', lineHeight: 1.6 }}>{step}</span>
            </li>
          ))}
        </ol>
          ...
          {guide.pricing.map((tier) => (
            <div key={tier.tierName} style={{ border: '1px solid var(--ink-a10)', borderRadius: 'var(--r-md)', padding: 16, background: '#FFFFFF' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                ...
                <span style={{ fontFamily: MONO_STACK, fontSize: 16, color: 'var(--ink)' }}>{tier.cost}</span>
              </div>
              <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 12px', display: 'flex', flexDirection: 'column', gap: 4 }}>
                {tier.keyLimits.map((limit, i) => (
                  <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 16, color: 'var(--ink)', lineHeight: 1.6 }}>{limit}</span>
                  </li>
                ))}
              </ul>
              <div style={{ paddingTop: 12, borderTop: '1px solid var(--ink-a10)' }}>
                <SectionLabel>Banking verdict</SectionLabel>
                <p style={{ fontSize: 16, color: 'var(--ink)', lineHeight: 1.6, margin: 0 }}>{tier.bankingVerdict}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          {guide.bankingUseCases.map((useCase) => (
            <div key={useCase.number} style={{ border: '1px solid var(--ink-a10)', borderRadius: 'var(--r-md)', padding: 16, background: '#FFFFFF', display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                <span style={{ fontFamily: MONO_STACK, fontSize: 11, flexShrink: 0, marginTop: 2, color: guide.colorVar }} aria-hidden="true">
                  {useCase.number}.
                </span>
                <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--ink)', lineHeight: 1.4, margin: 0 }}>
                  {useCase.title}
                </h3>
              </div>
              <div>
              ...
              <div>
                ...
                <p style={{ fontSize: 16, color: 'var(--ink)', lineHeight: 1.6, margin: 0 }}>{useCase.expectedOutput}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          {guide.customInstructions.available ? (
            <>
              <div>
                ...
                <p style={{ fontSize: 16, color: 'var(--ink)', lineHeight: 1.6, margin: 0 }}>{guide.customInstructions.howTo}</p>
              </div>
      ...
      <AccordionSection title="Data Safety for Banking Use" accentVar={guide.colorVar}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ padding: 16, borderLeft: `4px solid ${guide.colorVar}`, borderRadius: 'var(--r-md)', background: '#FFFFFF' }}>
            <p style={{ fontSize: 16, fontWeight: 600, color: 'var(--ink)', lineHeight: 1.6, margin: 0 }}>{guide.dataSafety.summary}</p>
          </div>
          <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
            {guide.dataSafety.details.map((detail, i) => (
              <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                <span style={{ marginTop: 7, width: 4, height: 4, borderRadius: '50%', background: guide.colorVar, flexShrink: 0 }} aria-hidden="true" />
                <span style={{ fontSize: 16, color: 'var(--ink)', lineHeight: 1.6 }}>{detail}</span>
              </li>
            ))}
          </ul>
          <div style={{ border: '1px solid var(--ink-a10)', borderRadius: 'var(--r-md)', padding: 16, background: '#FFFFFF' }}>
            <SectionLabel>Banking verdict</SectionLabel>
            <p style={{ fontSize: 16, color: 'var(--ink)', lineHeight: 1.6, margin: 0 }}>{guide.dataSafety.bankingVerdict}</p>
          </div>
        </div>
      </AccordionSection>
        ...
        <ol style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
          {guide.proTips.map((tip) => (
            <li key={tip.number} style={{ display: 'flex', gap: 16 }}>
              <span
                style={{ fontFamily: MONO_STACK, fontSize: 18, fontWeight: 700, flexShrink: 0, lineHeight: 1.2, color: guide.colorVar }}
                aria-hidden="true"
              >
                {tip.number}
              </span>
              <p style={{ fontSize: 16, color: 'var(--ink)', lineHeight: 1.6, paddingTop: 2, margin: 0 }}>{tip.tip}</p>
            </li>
          ))}
        </ol>
      </AccordionSection>
