...
export default async function AdminPage() {
  ...
  const completedUsers = enrollments.filter((row) => row.completed_modules.length >= FOUNDATION_PROGRAM_TOTAL_MODULES).length;
                ...
                return (
                  <tr key={row.id} className="border-t border-[color:var(--color-ink)]/10">
                    ...
                    <td className="px-4 py-3 font-mono text-[color:var(--color-terra)]">
                      {row.completed_modules.length}/{FOUNDATION_PROGRAM_TOTAL_MODULES}
                    </td>
                    <td className="px-4 py-3 text-[color:var(--color-slate)]">
