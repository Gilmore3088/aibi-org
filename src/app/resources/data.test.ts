...
describe('resources data model', () => {
  ...
  it('derives template cards from the canonical template registry', () => {
    const genericTemplates = templates.filter((template) => !template.roles?.length);

    expect(genericTemplates.map((template) => template.slug)).toEqual(
      TEMPLATE_INDEX.map((template) => template.slug),
    );
    expect(genericTemplates.map((template) => template.title)).toEqual(
      TEMPLATE_INDEX.map((template) => template.title),
    );
    expect(genericTemplates.map((template) => template.desc)).toEqual(
      TEMPLATE_INDEX.map((template) => template.dek),
    );
  });

  it('backs every manifest template Word download with a public manifest resource', () => {
    const manifestTemplates = templates.filter((template) =>
      template.download?.startsWith('/api/resources/templates/'),
    );
  ...
  it('includes the BSA/AML role-specific SAR template card', () => {
    const sarTemplate = templates.find((template) => template.slug === 'sar-narrative-template');

    expect(sarTemplate?.roles).toEqual(['bsa-aml']);
    expect(sarTemplate?.href).toBe('/playbooks/bsa-aml/sar-narrative-template');
    expect(sarTemplate?.download).toBe('/api/playbooks/bsa-aml/sar-narrative-template/word');
  });
