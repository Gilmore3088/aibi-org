...
export default async function TeamAssessmentAdminPage({ params }: PageProps) {
  const { cohortId } = await params;
  const nextPath = `/assessment/team/admin/${cohortId}`;
  ...
  return (
    <TeamAdminDashboard
      ...
      printUrl={`/assessment/team/admin/${cohort.id}/print`}
    />
  );
}
