...
export default async function ToolboxPage() {
  ...
  if (!hasFullToolboxAccess(access)) {
    if (hasStarterToolkitAccess(access)) {
      redirect('/dashboard/toolbox/starter');
    }
    return <Paywall />;
  }
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-6 py-6 lg:flex-row lg:items-end lg:justify-between lg:px-10">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-[color:var(--color-slate)]">
              Build durable AI skills, test them through the AIBI API proxy, save
              them to your account, and export Markdown files for your own
              repository.
            </p>
          </div>
          <Link
