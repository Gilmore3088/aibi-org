...
function deriveMetrics(enrollments: readonly MockEnrollment[]): CumulativeMetrics {
  ...
  // AiBI-P: ~2 skills per module, AiBI-S: ~4 departmental skills per module
  const skillsBuilt = pModules * 2 + sModules * 4;

  // AiBI-P: ~15 hrs/yr saved per module, AiBI-S: ~40 hrs/yr per module (departmental scope)
  const hoursSavedPerYear = pModules * 15 + sModules * 40;

  // AiBI-P: 1 workflow per 3 modules (rounded), AiBI-S: 1 per 2
  const workflowsAutomated = Math.floor(pModules / 3) + Math.floor(sModules / 2);

  // Quick wins: 1 per AiBI-P module completed past module 4
