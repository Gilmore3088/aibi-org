...
const DIR = join(root, "public", "footage", "_calc");
...
await wait(900);
await moveTo("#monthly"); await wait(1200);
// scroll the amortization schedule
await page.locator(".sched").hover();
for (let i = 0; i < 6; i++) { await page.mouse.wheel(0, 120); await wait(280); }
await wait(1500);
