...
| [ADDIE Foundation rebuild (`feature/addie-v1`)](../docs/Foundation-Course-ADDIE/AiBI_Start_Here.md) | (tracker is `docs/Foundation-Course-ADDIE/AiBI_Module_Production_Tracker.md` + `AiBI_Launch_Checklist.md`) | **Wave 1 + 2 + 3 done · final e2e drift audit pending** | Full ADDIE rebuild end-to-end on 2026-05-23: schema (`addie.*`, 19 tables + 2 views + 3 storage buckets), sandbox service (3 providers + failover + circuit breaker + all 8 §14 tests), auth/payments (gate fork + Stripe × 3 products + webhook + team seats), web shell (lesson player + Toolbox + gate UI + dashboard), all 6 modules (M0–M5: 24 lessons, 25 track variants, 55 KCs, 13 exercises including 3 LLM sandboxes), paid entitlement gate, $99 assessment surface (writes to `addie.assessment_results`; 8-dim model from `content/assessments/v2/`), team admin dashboard (FR-D4 counts-only). 20 commits ahead of main; 337/337 tests; tsc clean. Wave 1 audit: `AiBI_Wave_1_Audit_2026-05-23.md`. Final e2e drift audit + bridge-wiring for the on-main runner is the remaining work before merge. | — (branch-scoped, not on GitHub issue board) |
...
| [Foundations UX recovery · 2026-05-25](../Plans/aibi-foundations-ux-recovery-2026-05-25.md) | [`aibi-foundations-ux-recovery.md`](./aibi-foundations-ux-recovery.md) | **~52** | Active — Phase 0 closing, Phase 1 next | Four directional decisions locked (Shell extraction · Workbench Pack as M4 artifact · M5 stays Projects-and-Context, re-threaded · tagline kept). Branch `feature/addie-v2` (forked from `main` 3774d75). Synthesizes 2026-05-24 reviewer fleet (CEO Bill / CRO Margaret / Branch Mgr Devon / Sr PM Vera e2e) + comprehensive audit + 2026-05-25 product review. Phases: 1 = Guided Lesson Shell + M0–M3 migration (13 lessons, 4 workstreams) · 2 = Workbench Pack + M5 re-thread · 3 = Artifact Review Shell + leadership-track depth · 4 = funnel/systems wiring (Vera's findings). See DECISIONS 2026-05-25 entry. |
