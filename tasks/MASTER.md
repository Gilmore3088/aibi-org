...
| [Content engine (Scout + queue + admin UI)](../Plans/content-engine.md) | [`content-engine.md`](./content-engine.md) | **~70** | Plan + initial scaffold on `feature/content-engine` — code in [`/content-engine/`](../content-engine/). Supabase decisions resolved 2026-05-18: shared project, `content_engine` schema (see `DECISIONS.md`). | _no GH issue yet_ |
