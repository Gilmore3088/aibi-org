...
// Access: AiBI-S enrolled learners only. AiBI-Foundation completers can see these resources exist but cannot access them.
// These five frameworks are the primary justification for the $1,495 price difference between AiBI-Foundation and AiBI-S.
  ...
  sections: [
    {
      ...
      content: `AiBI-Foundation teaches you to build skills that do one thing well. AiBI-S introduces the next architectural question: what do you do when a valuable workflow requires multiple distinct AI steps in sequence?
...
The fourth behavior — peer teaching without prompting — is the strongest adoption signal. It means the skill library has become part of how the department talks about its work, not just a tool some people use.`,
    },
    {
      ...
      content: `In AiBI-Foundation, shadow AI refers to staff using AI tools without institutional knowledge — ungoverned use that creates data classification and compliance risk. AiBI-S inverts this insight: shadow AI is also an intelligence source.
  ...
  sections: [
    {
      ...
      content: `AiBI-Foundation teaches the RTFC framework — Role, Task, Format, Constraints — as the foundation of skill writing. Every skill you submitted for AiBI-Foundation certification used this structure. It works. But it handles linear tasks: one input, one transformation, one output.
...
The four additions — INPUT FORMAT, VALIDATION, PROCESS, and GOTCHA — are what distinguish a production skill from a prototype. AiBI-Foundation required Role, Task, Format, and Constraints. AiBI-S adds the four production components that make skills reliable enough for institutional use.`,
    },
    {
...
A skill that lists its context dependencies explicitly can be audited, maintained, and handed off. A skill with undocumented context dependencies is a liability — when a context file changes, no one knows which skills are affected.`,
    },
    {
      ...
      content: `Individual skill iteration — running a skill, reviewing the output, adjusting the prompt, running again — is the method AiBI-Foundation teaches. It is appropriate for personal-use skills where the developer and the user are the same person.
